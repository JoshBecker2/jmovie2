...
<body onload="loadHome()">
	...
	<div id="searchContainer">
		...
		<form action="search.php" method="GET" id="search">
			<input id="searchBar" type="text" name="sq" placeholder="Search for movies and shows...">
			<br><button type="button" id="liveTV" onclick="toggleLiveTV()">Live TV</button>
		</form>
		<button id="infoButton" onclick="menu('close')">?</button>
	</div>

	<div id="bodyContainer">
		<h2><y>Recently Viewed</y></h2>
		<script>
			viewed = JSON.parse(localStorage.getItem("viewed"));
			const container = document.getElementById("bodyContainer");
			if (viewed == null)
				container.innerHTML += "<h2>10 of your most recently viewed items will go here!</h2>"
			viewed.reverse().forEach(res => {
				if (res[2] == "live") { 
					block = `
						<div class="movie">
							<img class="poster" src="` + res[1] + `" width="150px" height="150px" alt="Station Image Failed to Load!" style="padding-bottom: 30px;"><br>
							<a class="movieTitle" href="player.php?url=` + res[3] + `&type=` + res[2] + `&title=` + res[0] + `&poster=` + res[1] + `"><i>` + res[0] + `</i></a>
					...
					block = `
						<div class="movie">
							<img style="margin-bottom: 20px;" class="poster" src="` + res[1] + `" width="150px" height="225px"><br>
							<a class="movieTitle" href="player.php?id=` + res[3] + `&type=` + res[2] + `&title=` + res[0] + `&poster=` + res[1] + `"><i>` + res[0] + `</i></a>
	...
	<script type="text/javascript">
		function generateSearchPlaceholder(state) {
			if (state == true) {
				titles = [
				"BBC",
				"ABC",
		...
		function loadHome() {	
			generateSearchPlaceholder(false);
		...
		function toggleLiveTV() {
			if (sessionStorage.getItem("liveTV") == "true") {
				...
				generateSearchPlaceholder(true);
